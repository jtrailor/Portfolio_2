      ...
      <div ref={ref} className="space-y-10">
        {skillGroups.map((group) => (
          <div key={group.category}>
            <h2 className="text-2xl font-semibold mb-4 text-brand-dark_blue dark:text-gray-200">
              {group.category}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {group.skills.map((skill, index) => (
                <TiltCard
                  key={skill.name}
                  className={`flex items-center gap-4 bg-brand-tan dark:bg-gray-700 rounded-lg shadow-md p-4 fade-in-up ${inView ? 'visible' : ''}`}
